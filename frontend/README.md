...
- 文档上传解析：上传单文件，查看分片与统计；可选是否直接入向量库，若后端未成功入库则自动**兜底索引一次**（避免再手动点“索引”）。
- 检索与对话：支持多知识库搜索、聊天 **SSE 流式回答**，并展示命中片段来源。
- 图片展示：支持展示“关联图片”，并支持在回答/片段中渲染 `[IMG:图片路径]` 的**原位置图片**（通过后端 `/api/images/...` 拉取）。
...
如需自定义后端地址，请在构建前修改 `app/lib/api.ts` 中的 `API_BASE_URL`（该值会被打包进前端产物）。
